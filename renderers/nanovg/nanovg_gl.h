...
static int glnvg__createShader(GLNVGshader* shader, const char* name, const char* header, const char* opts, const char* vshader, const char* fshader)
{
	...
	str[2] = vshader;
        qgl->glShaderSource(vert, 3, str, 0);
	str[2] = fshader;
        qgl->glShaderSource(frag, 3, str, 0);
	...
	if (status != GL_TRUE) {
        ...
        qgl->glBindAttribLocation(prog, 0, "vertex");
        qgl->glBindAttribLocation(prog, 1, "tcoord");
...
{
	if (shader->prog != 0)
                qgl->glDeleteProgram(shader->prog);
	if (shader->vert != 0)
                qgl->glDeleteShader(shader->vert);
	if (shader->frag != 0)
                qgl->glDeleteShader(shader->frag);
}

static void glnvg__getUniforms(GLNVGshader* shader)
{
        shader->loc[GLNVG_LOC_VIEWSIZE] = qgl->glGetUniformLocation(shader->prog, "viewSize");
        shader->loc[GLNVG_LOC_TEX] = qgl->glGetUniformLocation(shader->prog, "tex");

#if NANOVG_GL_USE_UNIFORMBUFFER
        shader->loc[GLNVG_LOC_FRAG] = qgl->glGetUniformBlockIndex(shader->prog, "frag");
#else
        shader->loc[GLNVG_LOC_FRAG] = qgl->glGetUniformLocation(shader->prog, "frag");
#endif
}
...
static void glnvg__blendCompositeOperation(NVGcompositeOperationState op)
{
	...
	if (srcRGB == GL_INVALID_ENUM || dstRGB == GL_INVALID_ENUM || srcAlpha == GL_INVALID_ENUM || dstAlpha == GL_INVALID_ENUM)
		glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA);
                ...
                qgl->glBlendFuncSeparate(srcRGB, dstRGB, srcAlpha, dstAlpha);
}
...
static void glnvg__renderFlush(void* uptr, NVGcompositeOperationState compositeOperation)
{
	...
	if (gl->ncalls > 0) {
		...
		// Setup require GL state.
                qgl->glUseProgram(gl->shader.prog);
		...
		glnvg__blendCompositeOperation(compositeOperation);
                ...
                qgl->glBindTexture(GL_TEXTURE_2D, 0);
		#if NANOVG_GL_USE_STATE_FILTER
		gl->boundTexture = 0;
		...
		// Set view and texture just once per frame.
                qgl->glUniform1i(gl->shader.loc[GLNVG_LOC_TEX], 0);
		...
		for (i = 0; i < gl->ncalls; i++) {
			...
			if (call->type == GLNVG_FILL)
                ...
                qgl->glDisableVertexAttribArray(0);
                qgl->glDisableVertexAttribArray(1);
#if defined NANOVG_GL3
